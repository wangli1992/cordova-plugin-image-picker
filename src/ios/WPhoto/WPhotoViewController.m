    ...
    if ([phoneVersion integerValue] >= 8) {
        ...
        [WPFunctionView getChoosePicPHImageManager:^(double progress) {
            ...
            if (!result) {
                ...
                if(cell.chooseStatus == NO&&(_chooseArray.count+_choosePhotoArr.count)== _selectPhotoOfMax){
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"小跟班提示您" message:@"最多只能选择9张图片" preferredStyle:UIAlertControllerStyleAlert];
                ...
                float size = data.length/(1024*1024*1.0);
                if(size>10){
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"小跟班提示您" message:@"选择的图片大于10M了,请重新选择" preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:@"知道了" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
