...
+(void)finishChoosePhotos:(finishChooseBlock)block chooseArray:(NSMutableArray *)_chooseArray
{
    ...
    if ([phoneVersion integerValue]>=8) {
        ...
        for (NSInteger i = 0; i<_chooseArray.count; i++) {
            ...
            //wl --转化为 base64字符串 传出去
              NSData *imageData = [WPFunc getSmallData:image];
            //        然后应用Base64编码将其转换成base64编码的字符串：
            //           NSString *encodedString = [imageData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
          
            //            NSLog(@"相册地址为：%@",encodedString);
             [myChoosePhotoArr addObject:imageData];
...
// ***********by wl 压缩图片大小
-(NSData *)getSmallData:(UIImage *)myimage{
